...
#define EXTRA_ENV_SETTINGS_DEFAULT                  \
	...
	"run mmcargs; "                                 \
	"run loadverity && source ${loadaddr} || reset; " \
	"run loadimage && bootm ${loadaddr}#${conf}; || reset; " \
	"elif test ${boot_src} = sd && test -n \"${mmcargs_trad}\"; then " \
	"run mmcargs_trad; "                            \
