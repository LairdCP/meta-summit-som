...
{
	DWORD dwActualSize = 0, dwActualLength = 0, dwSize = 0, i, j;
	char szFile[512];
...
{
	DWORD j =0, i = 0, k = 0, n = 0;
	WORD wAddress = 0;
	BYTE byRetVal = 0,  abyBuffer_read [SIZE_1KB];
	int m = 0, b = 1;
	DWORD error = Error_Success;
	bool gbSupportAll = TRUE, bReturn = 0;
	char szBuffer [260];
	int rc = FALSE;
...
BOOL WriteSPIFlash(int DevID,BYTE *byWriteData,int iRestartDelay,BOOL bErasePseudoOTP,DWORD dwStartAddr,DWORD dwSize )
{
	BOOL bReturn = FALSE;
	char szBuffer [260];
	libusb_device **devs;
	...
	if(gasHubInfo[DevID].sHubInfo.byFeaturesFlag & 0x01)
	{
		...
		error = SPI_SignatureErase(DevID,iRestartDelay);
...
{
	int bytes =0;
	BYTE abyjedecid_cmd[]={JEDEC_ID},abyjedecid_rsp[10],byRetVal=0,i;
	int rc = FALSE;
	DWORD error = Error_Success;
...
DWORD LoadBinary (int DevID,bool bErasePgmArea, BYTE *abyBuffer_SPI, DWORD dstart_address, DWORD dwActualSize)
{
	...
	DWORD j =0, i = 0, k = 0, n = 0, l = 0, dwBufSize, spi_location = dstart_address, MAX_DOWNLOAD_SIZE;
	WORD wAddress = 0, wSize = 0;
	BOOL byRetVal = 0;
	BYTE SPIUploadData_verify [SIZE_130KB] , abyBuffer_read [SIZE_1KB], byWaitState;
	int m = 0, b = 1, p= 0;
	BOOL bBytepgmSupported = FALSE;
	BOOL bFlashsst26vf064b = 0;
		
	char szFlashType[260], szCompareString[260] = "SST25VF020B",szCompareStringSST26VF064B[16] = "SST26VF064B";

	// Intialize before every download; as last state would be false
	BOOL bCheck_flag = TRUE, bReturn = 0;
...
DWORD SPI_SignatureErase(unsigned int DevID,int iRestartDelay)
{
	...
	memcpy(abyBuffer_Erase,abySPISignEraseData,sizeof(abySPISignEraseData));
