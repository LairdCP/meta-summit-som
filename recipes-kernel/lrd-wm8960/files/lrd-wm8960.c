...
#include "../sound/soc/codecs/ts3a227e.h"
...
struct imx_wm8960_data {
	...
	struct snd_soc_aux_dev imx_headset_dev;
			 ...
			 void *data)
{
	...
	struct imx_wm8960_data *imx_data = snd_soc_card_get_drvdata(jack->card);
			     ...
			     struct device_attribute *attr, char *buf)
{
	...
	if (snd_soc_dapm_get_pin_status(dapm, "Mic Jack"))
		strcpy(buf, "Mic Jack\n");
	else
		strcpy(buf, "Main MIC\n");
...
static int imx_wm8960_jack_ts3a227_init(struct snd_soc_component *component)
{
	...
	ret = snd_soc_card_jack_new(card, data->imx_hp_jack_pin.pin, 
		SND_JACK_HEADSET |
		SND_JACK_BTN_0 | SND_JACK_BTN_1 |
		SND_JACK_BTN_2 | SND_JACK_BTN_3,
		&data->imx_hp_jack, &data->imx_hp_jack_pin, 1);
	if (ret) {
		dev_warn(card->dev, "hp jack init failed (%d)\n", ret);
		return ret;
	}

	ret = ts3a227e_enable_jack_detect(component, &data->imx_hp_jack);
	if (!ret) {
		ret = device_create_file(card->dev, &dev_attr_headphone);
		if (ret)
			dev_warn(card->dev, "create hp attr failed (%d)\n", ret);

		ret = device_create_file(card->dev, &dev_attr_micphone);
		if (ret)
			dev_warn(card->dev, "create mic attr failed (%d)\n", ret);
	}

	return ret;
}

static int imx_hifi_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_card *card = rtd->card;
	struct imx_wm8960_data *data = snd_soc_card_get_drvdata(card);
	int ret = 0;
	...
	data->imx_hp_jack_gpio.gpio = of_get_named_gpio_flags(card->dev->of_node,
							      "hp-det-gpios", 0,
							      &data->hp_active_low);

	data->imx_mic_jack_gpio.gpio = of_get_named_gpio_flags(card->dev->of_node,
							       "mic-det-gpios", 0,
							       &data->mic_active_low);
	...
	if (gpio_is_valid(data->imx_hp_jack_gpio.gpio)) {
		...
		data->imx_hp_jack_gpio.invert = (data->hp_active_low &  OF_GPIO_ACTIVE_LOW) != 0;
	...
	if (gpio_is_valid(data->imx_mic_jack_gpio.gpio)) {
		if (!data->is_headset_jack) {
			...
			data->imx_mic_jack_gpio.invert = (data->mic_active_low &  OF_GPIO_ACTIVE_LOW) != 0;
			ret = imx_wm8960_jack_init(&data->card, &data->imx_mic_jack,
						   &data->imx_mic_jack_pin, &data->imx_mic_jack_gpio);
			if (ret) {
				dev_warn(card->dev, "mic jack init failed (%d)\n", ret);
				goto out;
			}
		}
...
out:
	snd_soc_jack_notifier_register(&data->imx_hp_jack, &hp_jack_nb);
...
static int imx_wm8960_late_probe(struct snd_soc_card *card)
{
	...
	snd_soc_component_update_bits(codec_dai->component, WM8960_ADDCTL4, 1, 1);

	/* Enable MIC BIAS */
	snd_soc_dapm_force_enable_pin(&card->dapm, "MICB");
...
static int imx_wm8960_probe(struct platform_device *pdev)
{
	...
	data->imx_wm8960_dai[0].init = imx_hifi_init;
	...
	data->imx_headset_dev.dlc.of_node = of_parse_phandle(pdev->dev.of_node,
		"headset-controller", 0);
	if (data->imx_headset_dev.dlc.of_node) {
		data->imx_headset_dev.init = imx_wm8960_jack_ts3a227_init;
		data->card.aux_dev = &data->imx_headset_dev;
...
static const struct of_device_id imx_wm8960_dt_ids[] = {
	{ .compatible = "lrd,imx-audio-wm8960", },
	{ .compatible = "lrd,imx7d-dvk-wm8960"  },
	{ /* sentinel */ }
};
...
MODULE_AUTHOR("Laird Connectivity, Inc.");
MODULE_DESCRIPTION("Laird Connectivity i.MX WM8960 ASoC machine driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:lrd-wm8960");
