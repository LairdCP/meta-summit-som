	...
	case IIO_CHAN_INFO_RAW:
		...
		case IIO_VOLTAGE:
			...
			case PAC193X_VBUS_3_ADDR:
				*val = chip_info->chip_reg_data.vbus[chan->channel];
				return IIO_VAL_INT;
		...
		case IIO_CURRENT:
			...
			case PAC193X_VSENSE_3_ADDR:
				*val = chip_info->chip_reg_data.vsense[chan->channel];
				return IIO_VAL_INT;
			default:
				return -EINVAL;
			}
		...
		case IIO_POWER:
			...
			case PAC193X_VPOWER_3_ADDR:
				*val = chip_info->chip_reg_data.vpower[chan->channel];
				return IIO_VAL_INT;
			default:
				return -EINVAL;
			}
	...
	case IIO_CHAN_INFO_AVERAGE_RAW:
		...
		case IIO_VOLTAGE:
			...
			case PAC193X_VBUS_AVG_3_ADDR:
				*val = chip_info->chip_reg_data.vbus_avg[chan->channel];
				return IIO_VAL_INT;
				...
				bool refresh_v, u32 wait_time)
{
	...
	if (ret < 0) {
		dev_err(&client->dev, "%s - cannot send byte to PAC193x 0x%02X reg\n",
		...
		bool do_rfsh, bool refresh_v, u32 wait_time)
{
	...
	if (do_rfsh) {
		...
		if (ret < 0) {
			dev_err(&client->dev, "%s - cannot end refresh towards PAC193x\n", __func__);
	...
	offset_reg_data = 0;
	chip_info->chip_reg_data.acc_count = mACC_COUNT(&chip_info->chip_reg_data.meas_regs[offset_reg_data]);
	...
	for (cnt = 0; cnt < chip_info->phys_channels; cnt++) {
		...
		if (((chip_info->chip_reg_data.ctrl_regs[PAC193X_CHANNEL_DIS_LAT_REG_OFF] << cnt) &	0x80) == 0) {
			...
			if (chip_info->chip_reg_data.bi_dir[cnt]) {
				...
				chip_info->chip_reg_data.vpower_acc[cnt] = mVPOWER_ACCs(&chip_info->chip_reg_data.meas_regs[offset_reg_data]);
			} else {
				/* uni-directional channel */
				chip_info->chip_reg_data.vpower_acc[cnt] = mVPOWER_ACCu(&chip_info->chip_reg_data.meas_regs[offset_reg_data]);
			}
			 ...
			 */
			samp_shift = get_count_order(samp_rate_map_tbl[((chip_info->chip_reg_data.ctrl_regs[PAC193X_CTRL_LAT_REG_OFF]) >> 6)]);
	...
	/* check if the channel is active, skip all fields if disabled */
		if (((chip_info->chip_reg_data.ctrl_regs[PAC193X_CHANNEL_DIS_LAT_REG_OFF] << cnt) & 0x80) == 0) {
			...
			if (chip_info->chip_reg_data.bi_dir[cnt]) {
				/* bi-directional channel */
				chip_info->chip_reg_data.vsense[cnt] = mVBUS_SENSEs(&chip_info->chip_reg_data.meas_regs[offset_reg_data]);
			} else {
				/* uni-directional channel */
				chip_info->chip_reg_data.vsense[cnt] = mVBUS_SENSEu(&chip_info->chip_reg_data.meas_regs[offset_reg_data]);
			}
			offset_reg_data += PAC193X_VBUS_SENSE_REG_LEN;
		}
	}
	...
	/* check if the channel is active, skip all fields if disabled */
		if (((chip_info->chip_reg_data.ctrl_regs[PAC193X_CHANNEL_DIS_LAT_REG_OFF] << cnt) & 0x80) == 0) {
			...
			if (chip_info->chip_reg_data.bi_dir[cnt]) {
				/* bi-directional channel */
				chip_info->chip_reg_data.vsense_avg[cnt] = mVBUS_SENSEs(&chip_info->chip_reg_data.meas_regs[offset_reg_data]);
			} else {
				/* uni-directional channel */
				chip_info->chip_reg_data.vsense_avg[cnt] = mVBUS_SENSEu(&chip_info->chip_reg_data.meas_regs[offset_reg_data]);
			}
			offset_reg_data += PAC193X_VBUS_SENSE_REG_LEN;
		}
	}
	...
	/* check if the channel is active, skip all fields if disabled */
		if (((chip_info->chip_reg_data.ctrl_regs[PAC193X_CHANNEL_DIS_LAT_REG_OFF] << cnt) & 0x80) == 0) {
			...
			if (chip_info->chip_reg_data.bi_dir[cnt]) {
				/* bi-directional channel */
				chip_info->chip_reg_data.vpower[cnt] = mVPOWERs(&chip_info->chip_reg_data.meas_regs[offset_reg_data]);
			} else {
				/* uni-directional channel */
				chip_info->chip_reg_data.vpower[cnt] = mVPOWERu(&chip_info->chip_reg_data.meas_regs[offset_reg_data]);
			}
...
void pac193x_read_reg_timeout(struct timer_list *t)
{
	...
	queue_work(system_unbound_wq, &chip_info->work_chip_rfsh);
}
					...
					struct iio_dev *indio_dev)
{
	...
	dev_info(&client->dev, ":%s: Active chip channels: %d\n", __func__, total_iio_chan);
	...
	for (cnt = 0; cnt < chip_info->phys_channels; cnt++) {
		if (chip_info->chip_reg_data.active_channels[cnt]) {
			...
			ch_sp[IIO_VOLTAVG].address = cnt + PAC193X_VBUS_AVG_0_ADDR;
			ch_sp[IIO_CRTAVG].channel = cnt;
			ch_sp[IIO_CRTAVG].scan_index = cnt;
			ch_sp[IIO_CRTAVG].address = cnt + PAC193X_VSENSE_AVG_0_ADDR;
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (!name) {
		dev_err(&client->dev, "DT parameter parsing returned an error\n");
		return -EINVAL;
	}
...
static int pac193x_remove(struct i2c_client *client)
{
	...
	del_timer_sync(&chip_info->tmr_forced_update);
